...
# The implementation of the CLCRP module
...
class Mask2FormerPixelDecoderEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # hidden_states = 0 #Ablation Studies 2 Remove MHSA
...
class Mask2FormerPixelLevelModule(nn.Module):
    ...
    def forward(self, pixel_values: Tensor, output_hidden_states: bool = False) -> Mask2FormerPixelLevelModuleOutput:
        ...
        hbs=int(bs/2)
        backbone_features_T1 = self.encoder(pixel_values[:hbs,:,:,:]).feature_maps
        backbone_features_T2 = self.encoder(pixel_values[hbs:,:,:,:]).feature_maps

        backbone_features=[]
        for i in range(len(backbone_features_T1)):
            # backbone_features.append(torch.abs(backbone_features_T1[i]-backbone_features_T2[i]))
            backbone_features.append(torch.cat([backbone_features_T1[i],backbone_features_T2[i]],dim=1))
        backbone_features=tuple(backbone_features)
...
class Mask2FormerMaskedAttentionDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # encoder_attention_mask=None #Ablation studies 1 remove masked attention
        """
        Args:
...
class Mask2FormerModel(Mask2FormerPreTrainedModel):
    ...
    def forward(
        ...
        batch_size, _, height, width = pixel_values.shape #b,t,c,h,w
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def get_loss_dict(
        ...
        # print(masks_queries_logits.shape,class_queries_logits.shape,len(mask_labels),len(class_labels))
